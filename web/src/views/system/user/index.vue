...
import { apiListRoles } from '@/api/role';
...
function handleStatusChange(row: { [key: string]: any }) {
  ...
  ElMessageBox.confirm('确认要' + text + '' + row.username + '用户吗?', '警告', {
    ...
    .then(() => {
      return apiUpdateUserStatus(row.username, row.status);
    })
    .then(() => {
...
function handleQuery() {
  state.loading = true;
  apiUserPage(state.queryParams).then(data => {
    state.loading = false;
...
function resetPassword(row: { [key: string]: any }) {
  ElMessageBox.prompt('请输入用户「' + row.username + '」的新密码', '重置密码', {
    ...
    .then(({ value }) => {
      ...
      apiAdminResetPassword(row.username, value).then(() => {
...
function submitForm() {
  dataFormRef.value.validate((valid: any) => {
    if (valid) {
      ...
      if (userId) {
        apiUserUpdate(userId, state.formData).then(() => {
          ElMessage.success('修改用户成功');
          closeDialog();
          handleQuery();
        });
      } else {
        apiUserSave(state.formData).then(() => {
...
function handleDelete(row: { [key: string]: any }) {
  const usernames = row.username || state.ids.join(',');
...
function handleDownloadTemplate() {
  apiExportTemplate().then((response: any) => {
...
function submitImportForm() {
  importFormRef.value.validate((valid: any) => {
    if (valid) {
      ...
      apiImportUser(deptId, roleIds, state.excelFile).then(response => {
...
function handleExport() {
  apiExportUser(queryParams.value).then((response: any) => {
    const blob = new Blob([response.data], {
      type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;charset=utf-8'
    });
    const a = document.createElement('a');
            ...
            <el-dropdown split-button style="margin-left: 12px">
              ...
              <template #dropdown>
                <el-dropdown-menu>
                  ...
                  <el-dropdown-item :icon="Download" @click="handleExport">导出用户</el-dropdown-item>
                </el-dropdown-menu>
              </template>
            </el-dropdown>
            <el-button :icon="Top" style="margin-left: 12px" @click="showImportDialog">导入</el-button>
          </div> -->
        </div>
      </template>
